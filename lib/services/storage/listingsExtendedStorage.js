...
export const breakVersionChain = (listingId1, listingId2) => {
  ...
  // Use resolved database IDs for comparison (same pattern as manualLinkStorage)
  const id1 = listing1.id;
  const id2 = listing2.id;
  const prevId1 = listing1.previous_version_id;
  const prevId2 = listing2.previous_version_id;

  let removed = false;

  // Check if listing1.previous_version_id points to listing2
  if (prevId1 === id2) {
    SqliteConnection.execute(`UPDATE listings SET previous_version_id = NULL WHERE id = @id`, { id: id1 });
    // Un-supersede listing2 (make it visible again)
    SqliteConnection.execute(`UPDATE listings SET is_superseded = 0 WHERE id = @id`, { id: id2 });
    removed = true;
  }
  // Check if listing2.previous_version_id points to listing1
  else if (prevId2 === id1) {
    SqliteConnection.execute(`UPDATE listings SET previous_version_id = NULL WHERE id = @id`, { id: id2 });
    // Un-supersede listing1 (make it visible again)
    SqliteConnection.execute(`UPDATE listings SET is_superseded = 0 WHERE id = @id`, { id: id1 });
    removed = true;
  }
  // Fallback: if neither condition matches, determine which was older and un-supersede it
  else {
    const date1 = listing1.published_at || listing1.created_at || 0;
    const date2 = listing2.published_at || listing2.created_at || 0;
    const olderListingId = date1 >= date2 ? id2 : id1;

    // Un-supersede the older listing so it shows in the overview again
    SqliteConnection.execute(`UPDATE listings SET is_superseded = 0 WHERE id = @id`, { id: olderListingId });
    removed = true;
  }
