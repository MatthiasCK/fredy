...
export function up(db) {
  ...
  // Fuzzy identity for tolerant property matching (detects re-listings with minor variations)
  if (!columnExists('listings', 'fuzzy_identity')) {
    db.exec('ALTER TABLE listings ADD COLUMN fuzzy_identity TEXT');
  }

  // Superseded flag to hide older versions from overview
  if (!columnExists('listings', 'is_superseded')) {
    db.exec('ALTER TABLE listings ADD COLUMN is_superseded INTEGER NOT NULL DEFAULT 0');
  }
  ...
  if (!indexExists('idx_listings_fuzzy_identity')) {
    db.exec('CREATE INDEX idx_listings_fuzzy_identity ON listings(fuzzy_identity)');
  }

  // Backfill: Mark listings that have a newer version pointing to them as superseded
  db.exec(`
    UPDATE listings
    SET is_superseded = 1
    WHERE id IN (
