...
/**
 * Remove a job by id. Listings are preserved by detaching them (setting job_id to NULL)
 * before the DELETE so the FK CASCADE does not remove them.
 * @param {string} jobId - Job id.
 * @returns {void}
 */
export const removeJob = (jobId) => {
  SqliteConnection.execute(`UPDATE listings SET job_id = NULL WHERE job_id = @id`, { id: jobId });
...
export const removeJobsByUserId = (userId) => {
  ...
  // Detach listings so FK CASCADE does not remove them
  SqliteConnection.execute(
    `UPDATE listings SET job_id = NULL WHERE job_id IN (SELECT id FROM jobs WHERE user_id = @user_id)`,
    { user_id: userId },
  );
