 ...
 * Remove a user by id.
 *
 * Listings are preserved by detaching them (setting job_id to NULL) before the
 * cascading DELETE removes the user's jobs.
 *
 * @param {string} userId - The id of the user to remove.
 * @returns {void}
 */
export const removeUser = (userId) => {
  // Detach listings so FK CASCADE on jobs does not remove them
  SqliteConnection.execute(
    `UPDATE listings SET job_id = NULL WHERE job_id IN (SELECT id FROM jobs WHERE user_id = @id)`,
    { id: userId },
  );
