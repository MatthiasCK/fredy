...
const PARAM_NAME_MAP = {
  ...
  // Rented property filter (for buy searches)
  rented: 'rented',
  // Additional filters
...
const REAL_ESTATE_TYPE = {
  ...
  'einfamilienhaus-kaufen': 'housebuy',
  'einfamilienhaus-mieten': 'houserent',
  'mehrfamilienhaus-kaufen': 'housebuy',
  'doppelhaushaelfte-kaufen': 'housebuy',
  'reihenhaus-kaufen': 'housebuy',
};
...
export function convertWebToMobile(webUrl) {
  ...
  // Add required mobile API parameters if not already set
  // Pagesize of 50 balances API efficiency with rate limiting
  // The provider handles pagination automatically to fetch all results
  if (!mobileParams.pagesize) {
    mobileParams.pagesize = 50;
  }
  if (!mobileParams.pagenumber) {
    mobileParams.pagenumber = 1;
  }
  if (!mobileParams.sorting) {
    mobileParams.sorting = 'standard';
  }
