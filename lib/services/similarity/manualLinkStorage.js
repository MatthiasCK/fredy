...
export function createManualLink(listingId, linkedListingId, userId = null) {
  ...
  try {
    ...
    // Wire the older listing into the previous_version_id chain.
    // Find the tail of the newer listing's chain (the oldest listing with no previous_version_id)
    // and set its previous_version_id to the older listing (or the head of the older listing's chain).
    const newerChainIds = new Set();
    let chainTailId = newerOfPair.id;
    while (true) {
      newerChainIds.add(chainTailId);
      ...
      chainTailId = current.previous_version_id;
    }

    // Find the head of the older listing's chain (newest listing not referenced as previous by others within its own chain)
    let olderChainHeadId = olderOfPair.id;
    while (true) {
      const newerInChain = SqliteConnection.query(`SELECT id FROM listings WHERE previous_version_id = @id`, {
        id: olderChainHeadId,
      })[0];
      if (!newerInChain || newerChainIds.has(newerInChain.id)) break;
      olderChainHeadId = newerInChain.id;
    }

    // Connect: tail of newer chain -> head of older chain (if not already connected and not creating a cycle)
    if (chainTailId !== olderChainHeadId && !newerChainIds.has(olderChainHeadId)) {
      SqliteConnection.execute(`UPDATE listings SET previous_version_id = @olderId WHERE id = @tailId`, {
        tailId: chainTailId,
        olderId: olderChainHeadId,
      });
    }
...
export function removeManualLink(listingId, linkedListingId) {
  ...
  if (result.changes > 0) {
    ...
    // Clear listing2's previous_version_id if it points to ANY listing in listing1's chain
    const chain1Ids = getChainIdsExcluding(id1, id2);
    if (prevId2 && chain1Ids.has(prevId2)) {
    ...
    // Clear any listing in the remaining chain whose previous_version_id points to listing2
    // This handles the case where listing2 is the tail that another listing was linked to
    for (const chainId of chain1Ids) {
      SqliteConnection.execute(
        `UPDATE listings SET previous_version_id = NULL WHERE id = @chainId AND previous_version_id = @unlinkedId`,
        { chainId, unlinkedId: id2 },
      );
    }
