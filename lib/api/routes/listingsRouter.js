...
import {
  ...
  getVersionHistory,
  breakVersionChain,
} from '../../services/storage/listingsExtendedStorage.js';
...
listingsRouter.get('/table', async (req, res) => {
  ...
  const normalizedDeleted = toBool(deletedFilter) === true;
  ...
  res.body = listingStorage.queryListings({
    ...
    deletedFilter: normalizedDeleted,
...
listingsRouter.post('/restore', async (req, res) => {
  const { ids } = req.body;
  try {
    if (Array.isArray(ids) && ids.length > 0) {
...
listingsRouter.get('/details/:id', async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) {
      res.statusCode = 400;
    ...
    logger.error(error);
    res.statusCode = 500;
    res.body = { message: 'Failed to get listing details' };
  }
  res.send();
});

// Get version history for a listing
listingsRouter.get('/versions/:id', async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) {
      res.statusCode = 400;
    ...
    res.body = { versions: history };
  } catch (error) {
    logger.error(error);
    res.statusCode = 500;
    res.body = { message: 'Failed to get version history' };
  }
  res.send();
});

// Break the version chain between two listings (unlink auto-linked versions)
listingsRouter.delete('/version-link', async (req, res) => {
  try {
    const { listingId, linkedListingId } = req.body || {};
    logger.debug('Breaking version chain:', { listingId, linkedListingId });
    ...
    const result = breakVersionChain(listingId, linkedListingId);
    logger.debug('Break version chain result:', result);

    if (!result.success) {
      res.statusCode = 400;
      res.body = { message: result.error || 'Failed to break version chain' };
      return res.send();
    }
    ...
    logger.error('Error breaking version chain:', error);
    res.statusCode = 500;
    res.body = { message: 'Failed to break version chain' };
  }
  res.send();
});
